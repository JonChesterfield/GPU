...
namespace hsa {
template <typename T, hsa_agent_info_t req> struct agent_get_info {
...
hsa_status_t getAttrRaw(hsa_amd_memory_pool_t Pool,
                        hsa_amd_memory_pool_info_t Kind, Ty &Value) {
...
template <typename C> hsa_status_t iterate_regions(hsa_agent_t agent, C cb) {
...
template <typename T, hsa_region_info_t req> struct region_get_info {
...
hsa_amd_memory_pool_t find_coarse_grain_global(hsa_agent_t agent) {
  ...
  bool found = false;
  guard(hsa::amd_agent_iterate_memory_pools(
      agent, [&](hsa_amd_memory_pool_t Pool) -> hsa_status_t {
        if (found) {
          return HSA_STATUS_SUCCESS;
        }
        ...
        guard(getAttrRaw(Pool, HSA_AMD_MEMORY_POOL_INFO_GLOBAL_FLAGS,
                         GlobalFlags));

        bool isCoarse =
            GlobalFlags & HSA_AMD_MEMORY_POOL_GLOBAL_FLAG_COARSE_GRAINED;
...
static hsa_status_t invoke_hsa_copy(void *dst, hsa_agent_t dst_agent,
  ...
  hsa_status_t err = hsa_amd_memory_async_copy(dst, dst_agent, src, src_agent,
                                               size, 0, NULL, sig);
