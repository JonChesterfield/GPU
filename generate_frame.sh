...
RESSUFFIX=CodeGen/WebAssembly/vararg-frame.ll
RES=$HOME/llvm-project/llvm/test/$RESSUFFIX


$HOME/llvm-build/llvm/bin/clang -cc1 -std=c23 -nostdsysteminc -triple wasm32-unknown-unknown -O1 -emit-llvm -xc $HOME/GPU/generate_frame.sh  -o $TMP
echo '; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py' > $RES
echo '; RUN: llc < %s -verify-machineinstrs  | FileCheck %s' >> $RES
cat $TMP >> $RES
sed -E -i \
  -e '/^source.filename.*$/d' \
  -e '/^; ModuleID.*$/d' \
